...
def extract_equ_type(it: lib.item.item):
    if not it.has_tag("equipment type"):
        return ""
    return it.get_single_tag_value("equipment type").split("\t")[0].strip("`[]")


def equ_is_avatar(it: lib.item.item):
    return (
        "avatar" == extract_equ_type(it)
...
def equ_name_need_filter(name: str) -> bool:
    return (
        "(旧)" in name
        or "(傀儡)" in name
        or "[活动" in name
...
def equ_need_filter(it: lib.item.item) -> bool:
    if not it.has_tag("name"):
        return True
    name = it.get_single_tag_value("name")
    return equ_name_need_filter(name.strip("`"))
